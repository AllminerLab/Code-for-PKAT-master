...
class PKAT(object):
    ...
    def _parse_args(self, data_config, pretrain_data, args):
        ...
        self.weight_size = eval(args.layer_size)#eval去掉字符串最外侧的引号,并按照Python的语法执行去掉引号后的字符内容
    ...
    def _build_weights(self):
        ...
        all_weights['add_w'] = tf.Variable(initializer([self.emb_dim, self.emb_dim]),
                                                    name='add_w',dtype=tf.float32)
        all_weights['add_b'] = tf.Variable(initializer([1, self.emb_dim]),
                                           name='add_b', dtype=tf.float32)
        ...
        self.weight_size_list = [self.emb_dim] + self.weight_size #[64,64]?
    ...
    def _build_model_phase_I(self):
        ...
        # ua_embeddings.size=[n_user, emb_dim] 再跟BERT中得到的User对应的[Mask]的表示进行结合，H.size = [n_user, emb_dim]
        # 采用add进行特征融合，即 sum_ua_embeddings = w1(ua_embeddings)+w2(H)

        sv = loadmat('yelp2018_bert20000.mat') #dataset
        H = sv["data"] #user_size emb
        H = tf.cast(H, tf.float32)
        zero = tf.zeros((H.shape[0], 112)) #112-16=96-32=64
        H_update = tf.nn.leaky_relu(tf.matmul(H, self.weights['add_w'])+self.weights['add_b'])
        # H_update = tf.nn.leaky_relu(tf.matmul(H, self.weights['add_w']))
        # H_update = H
        H_update = tf.concat([H_update,zero],axis=1)
        self.sum_ua_embeddings =tf.add(self.ua_embeddings, H_update)

        #self.sum_ua_embeddings = tf.add(tf.multiply(self.ua_embeddings, self.weights['add1']), tf.multiply(H, self.weights['add2']))
        self.u_e = tf.nn.embedding_lookup(self.sum_ua_embeddings, self.users)
        #self.u_e = tf.nn.embedding_lookup(self.ua_embeddings, self.users)
    ...
    def _get_kg_inference(self, h, r, pos_t, neg_t): #将h和t投影到r空间中
    ...
    def _create_bi_interaction_embed(self):
        ...
        for k in range(0, self.n_layers):
            ...
            print("bi_embeddings size n")
            print(bi_embeddings.shape)
            ...
            print("norm_embeddings size n")
            print(norm_embeddings.shape)

            print("all_embeddings size n")
            print(len(all_embeddings))
