...
class PKAT_loader(Data):
    ...
    def _get_relational_adj_list(self):
        ...
        def _np_mat2sp_adj(np_mat, row_pre, col_pre):
            ...
            # single-direction
            a_rows = np_mat[:, 0] + row_pre  #user id的数组[0 0 0 ... 70677 70678 70678]
            a_cols = np_mat[:, 1] + col_pre  #item id加上user数量的数组 [93114 78139 71453 ... 81256 88544 91236]
            a_vals = [1.] * len(a_rows) #得到长度为a_rows的数组[1,1,...]

            b_rows = a_cols
            b_cols = a_rows
            b_vals = [1.] * len(b_rows)

            a_adj = sp.coo_matrix((a_vals, (a_rows, a_cols)), shape=(n_all, n_all)) #sp.coo_matrix用于生成坐标格式的矩阵
        ...
        adj_mat_list.append(R_inv)  #将两个转置的矩阵拼接在一起
        adj_r_list.append(self.n_relations + 1)  #有39个关系 adj_r_list=[0, 40]
    ...
    def _get_all_kg_dict(self):
        ...
        return all_kg_dict   #({...73088: [(70653, 40)], 92046: [(70664, 40)]})
