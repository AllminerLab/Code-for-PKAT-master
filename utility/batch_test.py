...
from .loader_pkat import PKAT_loader
...
if args.model_type in ['pkat']:
    data_generator = PKAT_loader(args=args, path=args.data_path + args.dataset)
...
def test(sess, model, users_to_test, drop_flag=False, batch_test_flag=False):
    ...
    for u_batch_id in range(n_user_batchs):
        ...
        if batch_test_flag:
            ...
            rate_batch = rate_batch.reshape((-1, len(item_batch))) #-1代表不限行数
